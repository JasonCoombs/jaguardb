...
JAGSOCK rayconnect( const char *host, unsigned int port, int timeoutsecs, bool errPrint )
{
	...
	JAGSOCK sockfd = JagNet::connect2Host( host, port, timeoutsecs, errPrint );
	return sockfd;
}
...
JAGSOCK JagNet::connect2Host( const char *host, short port, int timeoutsecs, bool errPrint )
{
    ...
    if (p == NULL) {
		...
		***/
		if ( errPrint ) { printf("E3088 Unable to connect to [%s:%d]\n", host, port ); fflush( stdout ); }
		return INVALID_SOCKET;
    }
...
void JagNet::_getIPFromHostName( const char *hostname, char *ip )
{
	...
	if ( strlen(ip) > 4 ) {
		// prt(("s2239 got host from getIPFromEtcFile hostname=%s ip=%s\n", hostname, ip ));
		return;
	}
...
JAGSOCK JagNet::acceptTimeout( JAGSOCK sock, struct sockaddr *pcliaddr, socklen_t* pclilen, jagint thrdgrp, int timeOutseconds )
{
	int connfd = 0;
	if ( 0 == thrdgrp ) {
		prt(("s2266 POSIX accept ... \n" ));
		connfd =  accept( sock, pcliaddr, pclilen);
		prt(("s2267 acceptTimeout returned accept connfd=%d \n", connfd ));
		return connfd;
	}
	...
	while ( 1 ) {
		...
		if ( -1 == connfd ) {
			if ( EAGAIN == errno || EWOULDBLOCK == errno ) {
				...
				prt(("s4801  accept none, try again ...\n" ));
				++cnt;
			} else {
				break;
			}
