...
JagCompFile *JagFSMgr::openf( JagDiskArrayFamily *fam, const AbaxString &fpath, jagint klen, jagint vlen, bool force )
{
	...
	if ( force || exist( fpath.c_str() ) ) {
		prt(("s10029 JagFSMgr::open() new JagCompFile klen=%d vlen=%d\n", klen, vlen ));
...
/***
// make fullpath dir
int JagFSMgr::mkdir( const AbaxString &fpath )
{
...
// delete fullpath dir
int JagFSMgr::rmdir( const AbaxString &fpath )
{
...
jagint JagFSMgr::pread(  const JagCompFile *compf, void *buf, size_t count, jagint offset)
{
	return compf->pread( (char*)buf, offset, count );
}

jagint JagFSMgr::pwrite( JagCompFile *compf, const void *buf, size_t count, jagint offset)
{
	return compf->pwrite( (char*)buf, offset, count );
}
