...
{
    public: 
        inline JagHeapData&  operator= ( const JagHeapData &d2 ) {
            keyval = d2.keyval;
            col = d2.col;
            return *this;
        }
        inline int ge (const JagHeapData<PairType>  &d2 ) const { 
            return ( keyval.ge( d2.keyval ) );
        }       
        inline int gt (const JagHeapData<PairType>  &d2 ) const { 
            return ( keyval.gt( d2.keyval  ) );
        }       
        inline int le (const JagHeapData<PairType>  &d2 ) const { 
            return ( keyval.le ( d2.keyval ) );
        }       
        inline int lt (const JagHeapData<PairType>  &d2 ) const { 
            return ( keyval.lt(  d2.keyval ) );
        }       

		inline void pointKey( const PairType &pair ) {
			keyval = pair;
		}
