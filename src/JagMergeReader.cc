...
JagMergeReader::JagMergeReader( JagVector<OnefileRange> &fRange, int veclen, int keylen, int vallen )
	:JagMergeReaderBase(veclen, keylen, vallen )
{
	_vec = new JagBuffReaderPtr[veclen];
	for ( int i = 0; i < veclen; ++i ) {
		_vec[i] = new JagBuffReader( (JagDiskArrayBase*)fRange[i].darr, fRange[i].readlen, 
...
bool JagMergeReader::getNext( char *buf, abaxint &pos )
{
	if ( _cacheBuf ) {
		pos = 0;
...
bool JagMergeReader::getRestartPos()
{
	if ( !_setRestartPos ) return false;
	for ( int i = 0; i < _veclen; ++i ) {
