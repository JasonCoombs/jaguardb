...
JagTableOrIndexAttrs::~JagTableOrIndexAttrs() 
{
	...
	if ( schAttr ) {
		...
		schAttr = NULL;
		numschAttr = 0;
		//prt(("s0334 schAttr=%0x deelte [] it done \n", schAttr ));
	}
}
...
{
	//prt(("s8301 JagTableOrIndexAttrs:: copy ctor ...\n" ));
	copyData( o );
	schAttr = NULL;
	numschAttr = 0;
	
	if ( o.numschAttr > 0 ) {
		//prt(("s1034  o.numschAttr=%d new JagSchemaAttribute[]\n", o.numschAttr ));
		schAttr = new JagSchemaAttribute[o.numschAttr];
		for ( int i = 0; i < o.numschAttr; ++i ) {
			schAttr[i] = o.schAttr[i];
			//prt(("s3036 schAttr[%d] = o.schAttr[i]\n", i ));
		}
...
JagTableOrIndexAttrs& JagTableOrIndexAttrs::operator=( const JagTableOrIndexAttrs &o )
{
	...
	//prt(("s8301 JagTableOrIndexAttrs::operator= ...\n" ));
	copyData( o );
	...
	if ( o.numschAttr > 0 ) {
		//prt(("s2034  asign o.numschAttr=%d new JagSchemaAttribute[]\n", o.numschAttr ));
		schAttr = new JagSchemaAttribute[o.numschAttr];
		for ( int i = 0; i < o.numschAttr; ++i ) {
			schAttr[i] = o.schAttr[i];
			//prt(("s3036 schAttr[%d] = o.schAttr[i]\n", i ));
