...

{
	...
	//prt(("s2183 addRole uid=%s role=[%s]\n", userid.c_str(), role.c_str() ));
	...
	if ( key.size() > KLEN ) {
		//prt(("E3641 addRole key=[%s] too long\n", key.c_str() ));
		return false;
	}
	...
	const JagParseParam &parseParam, int n, Jstr &rowFilter, Jstr &errmsg )
{
		...
		***/
		if ( JAG_COUNT_OP == parseParam.opcode && srec  ) {
    		...
    		for ( int j = 0; j < srec->columnVector->size(); ++j ) {
    			...
    			rc = isAuthed( op, req.session->uid, db, tab, col, oneFilter );
				/***
    			prt(("s3504 isAuthed rc=%d db=[%s] tab=[%s] col=[%s] oneFilter=[%s]\n", 
    		...
    		for ( int i = 0; i < parseParam.selColVec.size(); ++i ) {
    			// prt(("s3283 parseParam.selColVec[i].colList=[%s]\n", parseParam.selColVec[i].colList.c_str() ));
		...
		for ( int i = 0; i < parseParam.objectVec.size(); ++i ) {
			...
			col = "*";
			rc = isAuthed( op, req.session->uid, db, tab, col, rowFilter );
			//prt(("s4505 isAuthed rc=%d db=[%s] tab=[%s]\n", rc, db.c_str(), tab.c_str() ));
			if ( !rc ) {
	...
	//prt(("s4508 final isAuthed rc=true\n" ));
	return true;
}
						  ...
						  const Jstr &col, Jstr &rowFilter )
{
	...
	//prt(("s4003 check rowKey=[%s]\n", rowKey.c_str() ));
	//prt(("s4003 check rowKey2=[%s]\n", rowKey2.c_str() ));
	...
	//prt(("s4004 check rowKey=[%s]\n", rowKey.c_str() ));
	//prt(("s4004 check rowKey2=[%s]\n", rowKey2.c_str() ));
	if ( _hashmap->getValue( rowKey, value ) ) {
		p = value.c_str();
		if ( '\0' == *p ) { rowFilter = ""; } else { rowFilter = p; }
		return true;
	...
	rowKey = userid + "|" + db + "|*|*|" + op;
	rowKey2 = userid + "|" + db + "|*|*|*";
	//prt(("s4005 check rowKey=[%s]\n", rowKey.c_str() ));
	//prt(("s4005 check rowKey2=[%s]\n", rowKey2.c_str() ));
	if ( _hashmap->getValue( rowKey, value ) ) {
		p = value.c_str();
		if ( '\0' == *p ) { rowFilter = ""; } else { rowFilter = p; }
		return true;
	...
	rowKey = userid + "|*|*|*|" + op;
	rowKey2 = userid + "|*|*|*|*";
	//prt(("s4006 check rowKey=[%s]\n", rowKey.c_str() ));
	//prt(("s4006 check rowKey2=[%s]\n", rowKey2.c_str() ));
	if ( _hashmap->getValue( rowKey, value ) ) {
		p = value.c_str();
		if ( '\0' == *p ) { rowFilter = ""; } else { rowFilter = p; }
		return true;
