...
class JagCompFile
{
  public:
	...
	~JagCompFile();
	jagint pread(char *buf, jagint offset, jagint len ) const;
	jagint pwrite(const char *buf, jagint offset, jagint len );
	...
	//jagint writeToOneFile(const char *buf, jagint len );
	//jagint writeToMultiFile(const char *buf, jagint len );
	...
	float 	computeMergeCost( const JagDBMap *pairmap, jagint seqReadSpeed, jagint seqWriteSpeed, 
							  ...
							  /**
	float 	computeMergeCost( const JagDBMap *pairmap, jagint seqReadSpeed, jagint seqWriteSpeed, 
							  std::vector<JagMergeSeg> &vec );
							  ***/
	jagint 	mergeBufferToFile( const JagDBMap *pairmap, const JagVector<JagMergeSeg> &vec );
	// jagint 	mergeBufferToFile( const JagDBMap *pairmap, const std::vector<JagMergeSeg> &vec );
