...
int JagHotSpot<Pair>::updateUpsAndDowns( const Pair &dbpair )
{
	...
	// prt(("s3834 updateUpsAndDowns  this=%0x  thrd=%lld\n", this, THREADID ));
	if ( dbpair > _oldPair ) {
		++ _ups;
...
int JagHotSpot<Pair>::insert( abaxint usec, double spot, const Pair &dbpair )
{
	...
	hpair.key = usec;
	hpair.value = spot;
	// prt(("s8381 hotspot insert %.2f  this=%0x THREAD=%lld\n", spot, this, THREADID ));
...
int JagHotSpot<Pair>::goingRight() const
{
	...
	if ( res >= 0.7 ) {
		// prt(("s3822 goingRight  ups=%lld  downs=%lld this=%0x thrd=%lld res >=0.7\n", _ups, _downs, this, THREADID ));
		return 1;
	} else {
		// prt(("s3823 goingRight  ups=%lld  downs=%lld this=%0x thrd=%lld\n", _ups, _downs, this, THREADID ));
		/***
		if ( _ups/_downs >= 7 ) {
			return 1;
		}
